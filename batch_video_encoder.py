...
class BatchVideoEncoder:
    ...
    def encode_start(self):
        if not os.path.exists("Recovery.json"):
            wait_list = self.get_every_video_path(self.get_save_path(self.file_path))
        else:
    ...
    def get_every_video_path(self, encode_path: str) -> list:
        filelist = []
        ...
        with open("Recovery.json", "w", encoding="utf-8") as recovery_file:
            json.dump(res, recovery_file, ensure_ascii=False)
        return res
...
if __name__ == "__main__":
    ...
    if os.path.exists("Recovery.json"):
        ...
        worker_01.file_path = os.path.abspath(input("Please input the directory you want to bulk encode: "))
